...
module.exports = {
		...
		fs.readdir(self.directory, function(err, list) {
			...
			list.forEach(function(file) {
				...
				for(var key in controller) {
					...
					var args = self.translateFunctionBodyToArgArray(f);
					...
					aliases.forEach(function(alias) {
						...
						if(path !== false) {
							...
							var pathObj = {path: path, params: params, args: args, f: f};
							paths.push(pathObj);
						}
					});
				}
				...
				paths.forEach(function(pathObj){
					...
					self.bindFunction(app, pathObj.path, pathObj.params, pathObj.args, pathObj.f);
				})
			});
			if(cb) {
				cb();
	...
	bindFunction : function(app, path, params, args, f) {
	...
	translateFunctionBodyToArgArray : function(f) {
	...
	translateFunctionBodyToParameterArray : function(f) {
		...
		if (typeof f == 'function') {
			...
			if(params.length >= 3 && params.indexOf('next') > -1 ) {
				params.splice(0,3);
				return params;
			}
